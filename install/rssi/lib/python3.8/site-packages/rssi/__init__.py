...
class RSSI_Scan(object):
    ...
    def getFrequency(raw_cell):
        frequency = raw_cell.split('Frequency:')[1]
        frequency = float(frequency.split(' ')[0])*1000
        return frequency
    @staticmethod
    ...
    # filterAccessPoints
        ...
        # Returns: (Array of dictionaries)
            ...
            # ] 
    @staticmethod
    def filterAccessPoints(all_access_points,network_id,network_mac):
        ...
        for point in all_access_points:
            ...
            if (point['mac'] in network_mac):
               focus_points.append(point)
            if (point['ssid'] in network_id) and point['ssid']!='':
               focus_points.append(point)
        return focus_points
    ...
    # getAPinfo
        ...
        # Returns: (Array of dictionaries)
            ...
            #     }
            # ] 
    def getAPinfo(self,networks=False,networks_mac=False,sudo=False):
        ...
        if all_access_points:
           if networks or networks_mac:
            # Checks if specific networks were declared.
                if networks :
                    # Return specific access-points found.
                    return self.filterAccessPoints(all_access_points, networks,'huong998:@')
                if networks_mac:
            	     return self.filterAccessPoints(all_access_points, 'huong998:@',networks_mac)
           else:
